...
class WriterBook(tk.Toplevel):
    ...
    def list_view_mini(self):
        ...
        # 下の方にあるscroll_view()関数でself.canvas_liにframeを描画する処理とスクロール量の算出を行う    
        #--------------
    ...
    '''
     ...
     戻り値は、データ件数（数値）
    '''
    def _gui_data(self, result:tuple)->int:
        # 項目ごとにリストにとる
        ...
        for i, (id, title, publisher) in enumerate(zip(ids, titles, publishers)):
            ...
            # publisher Entry
            text_list_publisher = tk.Entry(self.frame_body, width=20)
            ...
            # 詳細ボタン
            # command=_click_detailではなく、bindを使うとrow（何行目）が取得できる
