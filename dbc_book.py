...
class Book(BookData):
    ...
    def list(self,sort:str,state:str) ->tuple[tuple,tuple[int]]:
        ...
        try:
            with self.cursor() as cur:    
                ...
                # self.logger_name.info("LISTを表示。 ")
                return result,count_all
        except Exception as e:
            self.logger_name.error("LISTを表示に失敗 ")
    ...
    def detail(self,id:int)->tuple:
        ...
        tables ='book_table LEFT JOIN writer_table ON book_table.writer = writer_table.id LEFT JOIN publisher_table ON book_table.publisher = publisher_table.id' 
        sql = f"SELECT {columns} FROM {tables} WHERE book_table.id = %s"
        try:
            with self.cursor() as cur: 
                cur.execute(sql, (id,) )
                result = cur.fetchall()
    ...
    def insert(self,title:str,rubi:str,writer:int,publisher:int,memo:Optional[str],state:Literal['未読', '読書中', '既読'])->tuple:
        ...
        try:
            with self.cursor() as cur: 
                ...
                cur.execute(sql_id )
                result = cur.fetchone()
                self.logger_name.info(f"INSERT: {title}を追加。 ")
                return result    
        except Exception as e:
            print('データベースの接続に失敗しました。',e)
            self.logger_name.error(f"{title}の追加に失敗。 {e}", exc_info=True)
    ...
    def update(self,title:str,rubi:str,writer:int,publisher:int,memo:Optional[str],state:Literal['未読', '読書中', '既読'],id:int):
        ...
        try:
            with self.cursor() as cur: 
                cur.execute(sql, val)
                self.logger_name.info(f"UPDATE: {title}のデータを変更。 ")

        except Exception as e:
            print('データベースの接続に失敗しました。',e)
            self.logger_name.error(f"{title}のデータ更新に失敗。 {e}", exc_info=True)
    ...
    def delete(self,id:int, title:str):
        ...
        try:
            with self.cursor() as cur: 
                cur.execute(sql, (id,))
                self.logger_name.info(f"Delete: {title}を削除。")

        except Exception as e:
            print('データベースの接続に失敗しました。',e)
            self.logger_name.error(f"{title}の削除に失敗 {e}", exc_info=True)
